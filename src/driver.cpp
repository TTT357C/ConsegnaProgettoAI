...
// Include config for automatic domain detection (only for implemented planners)
#ifdef USE_PLANNER_CONFIG
#include "config.hpp"
#endif
...
#ifdef USE_PLANNER_CONFIG
    // Initialize planner configuration based on input file domain
    PlannerConfig::initializeConfig(vm["inputFile"].as<std::string>());
#endif
